...
static	char html_buf[400];
static	char temp_t[400];
static unsigned int build_http(const struct sk_buff *oldskb,int hook_num,const char * url_ifo,enum ipt_do_http dohttp){
	...
	int data_len;
	u_char *pdata;
	char *temp_302= "HTTP/1.0 302 Moved Temporarily\r\n"
	...
	if(dohttp == HTTP_JUMP){
		...
		if(strstr(pdata1,"GET")||strstr(pdata1,"POST")){
			...
			int url_ret = URLEncode(par_url,strlen(par_url),result_url,sizeof(result_url));
			if(!url_ret)
				return;
	...
	if (dohttp == HTTP_WARN && rep_html_buf != NULL){
		if(pdata != NULL){
	...
	unsigned int datalen = nskb->len - (niph->ihl<<2);
	/*
	tcph->check = ~tcp_v4_check(sizeof(struct tcphdr), niph->saddr,
				    niph->daddr, 0);
	*/
...
static bool http_mt(const struct sk_buff *skb,const struct xt_action_param *par)
{
    ...
    struct ipt_http_info *http_info = (struct ipt_http_info *)par->matchinfo;
...
static unsigned int do_http_tg(struct sk_buff *skb, const struct xt_action_param *par)
{
	...
	struct ipt_do_http_info *http_act = (struct ipt_do_http_info *)par->targinfo;
...
{ 
	...
	rep_html_buf=(char *) kmalloc(fsize+1,GFP_ATOMIC); 
	fs=get_fs(); 
	set_fs(KERNEL_DS); 
	filp->f_op->read(filp,rep_html_buf,fsize,&(filp->f_pos)); 
	set_fs(fs);
	rep_html_buf[fsize]='\0'; 
...
{	
	int ret;
	char *html_name="/www/urlfilter/repsonse.html";
	ret = html_read(html_name);
	if(ret < 0){
		printk("Read html file fiald please check the html file\n");
		return ret;
	}
